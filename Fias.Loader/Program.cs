...
namespace Fias.Loader
{
    internal static class Program
    {
        ...
        private static int GetRecordCount(Schema schema)
        {
            ...
            using (var sqlConnection = GetNewOpenedSqlConnection())
            {
                ...
                */
                var getCountQuery = string.Format(@"SELECT row_count
				FROM sys.dm_db_partition_stats
				WHERE object_name(object_id) = '{0}'", schema.XmlElement);
        ...
        private static string GenerateSqlForCreateTable(Schema schema)
        {
            ...
            var tableName = schema.XmlElement;
        ...
        private static string WrapValue(Field field, string value)
        {
            ...
            {
                wrappedValue = string.IsNullOrWhiteSpace(value) || value == "1900-01-01"
                                ? "null"
                                : String.Format("convert(datetime,'{0}',120)", value);
            }
            else
            {
                wrappedValue = string.IsNullOrWhiteSpace(value) ? "null" : string.Format(field.Type.Format, value.Replace("'", "''"));
            }
            return wrappedValue;
        }
        ...
        private static void ExecuteInserts(Schema schema, int count, List<string> rowSql, int fieldCount)
        {
            ...
            var insertSql = new StringBuilder(string.Format("INSERT INTO [{0}] (", schema.XmlElement));
